...
std::vector<float> CameraNormal::m_tv_radius2;
float CameraNormal::m_tv_default_radius2 = 0.0f;
    ...
    case CM_SPECTATOR_TV:
        {
            ...
            {
                Vec3 ball_pos = soccer_world->getBallPosition();
                // Select the best TV camera (immediate switch)
                ...
                if (m_tv_radius2.size() != m_tv_cameras.size())
                    m_tv_radius2.assign(m_tv_cameras.size(), m_tv_default_radius2);

                for (unsigned i = 0; i < m_tv_cameras.size(); i++)
                {
                    float d2 = (m_tv_cameras[i] - ball_pos).length2();
                    float r2 = 0.0f;
                    if (i < m_tv_radius2.size()) r2 = m_tv_radius2[i];
                ...
                {
                    // Switch immediately to the best candidate (prefer in-radius)
...
void CameraNormal::readTVCameras(const XMLNode &root)
{
    ...
    m_tv_default_radius2 = (radius > 0.0f) ? (radius * radius) : 0.0f;
    ...
    for (unsigned int i = 0; i < root.getNumNodes(); i++)
    {
        ...
        if (is_tv_camera_node)
        {
            ...
            if (child_radius > 0.0f)
                m_tv_radius2.push_back(child_radius * child_radius);
            else
                m_tv_radius2.push_back(m_tv_default_radius2);
        }
    }
} // readTVCameras
